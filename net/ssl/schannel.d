...
debug(ae_unittest) private static immutable ubyte[] testPfxBytes = [
...
// A real curl.exe sends a substantially larger ClientHello than the SChannel
// client below. Verify that a PFX-backed server completes that handshake and
// can serve a decrypted HTTP request from the standard Windows curl client.
debug(ae_unittest) unittest
{
    ...
    enum curlPath = r"C:\Windows\System32\curl.exe";
    if (!curlPath.exists)
        return;
    ...
    s.handleAccept = (TcpConnection c)
    {
        ...
        a.handleReadData = (Data d) {
            request ~= cast(string)d.unsafeContents;
            if (!request.endsWith("\r\n\r\n"))
                return;
            assert(request.startsWith("GET / HTTP/1.1\r\n"),
                "unexpected decrypted curl.exe request: " ~ request);
            gotRequest = true;
            a.send(Data(("HTTP/1.1 200 OK\r\n"
                ~ "Content-Length: 2\r\n"
                ~ "Connection: close\r\n\r\n"
                ~ "OK").asBytes));
            a.disconnect();
    ...
    auto curl = spawnProcess([
        ...
        "--max-time", "10",
        "--output", "NUL",
        "https://127.0.0.1:" ~ port.to!string ~ "/",
    ]);
    int curlStatus = -1;
    asyncWait(curl, (int status) {
        curlStatus = status;
        s.close();
    });
    ...
    assert(accepted, "server did not accept curl.exe's TCP connection");
    assert(curlStatus == 0,
        "curl.exe HTTPS request failed with exit status " ~ curlStatus.to!string);
    assert(gotRequest, "curl.exe completed without sending an HTTP request");
}

// An adapter built for an already-connected transport must report handshake
// initialization failures to its creator, before a disconnect handler exists.
debug(ae_unittest) unittest
{
    import std.exception : assertThrown;

    static class ThrowingConnection : IConnection
    {
        @property ConnectionState state() { return ConnectionState.connected; }
        void send(scope Data[] data, int priority = DEFAULT_PRIORITY)
        {
            throw new Exception("test transport send failure");
        }
    ...
    assertThrown!Exception(p.createAdapter(cc, new ThrowingConnection));
}
