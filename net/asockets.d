...
// === crash-localization v2: run ae.net.asockets FIRST, then the rest (do not merge) ===
version (unittest)
{
	shared static this()
	{
		import core.runtime : Runtime;
		Runtime.moduleUnitTester = function bool() {
			import core.stdc.stdio : fprintf, fflush, stderr;
			static void runOne(ModuleInfo* m) {
				if (m is null) return;
				auto fp = m.unitTest;
				if (fp is null) return;
				fprintf(stderr, "UT-BEGIN %.*s\n", cast(int) m.name.length, m.name.ptr);
				fflush(stderr);
				fp();
				fprintf(stderr, "UT-END   %.*s\n", cast(int) m.name.length, m.name.ptr);
				fflush(stderr);
			}
			// Phase 1: the DNS-churn module first
			foreach (m; ModuleInfo)
				if (m !is null && m.name == "ae.net.asockets")
					runOne(m);
			// Phase 2: everything else, in natural order
			foreach (m; ModuleInfo)
				if (m !is null && m.name != "ae.net.asockets")
					runOne(m);
			return true;
